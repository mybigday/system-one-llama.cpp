...
int llama_server(common_params & params, int argc, char ** argv) {
    ...
    if (is_router_server) {
        ...
        routes.post_system_one             = models_routes->proxy_post;
    ...
    ctx_http.post("/systemone",                ex_wrapper(routes.post_system_one));
    ctx_http.post("/v1/systemone",             ex_wrapper(routes.post_system_one)); // Jev-compatible typed decisions
